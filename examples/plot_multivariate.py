...
M = (4, 5)
closed = (False, False)
control_points = [np.sin(np.linspace(0, np.pi, m + 2)) for m in M]
...
t = np.stack(np.meshgrid(*(np.linspace(0, M[i], 100) for i in range(len(control_points))), indexing="ij"), axis=-1)
...
x = np.sin(np.linspace(0, 2 * np.pi, M[0] + 1))[:-1]
y = np.cos(np.linspace(0, 2 * np.pi, M[0] + 1))[:-1]
z = np.ones(M[0])
...
control_points = [control_points_circular, control_points_straight]
...
trivariate_control_points = [np.sin(np.linspace(0, np.pi, m + 2)) for m in trivariate_M]
